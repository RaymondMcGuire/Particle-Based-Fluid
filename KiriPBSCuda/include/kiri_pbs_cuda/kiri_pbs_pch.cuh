 ...
 * @LastEditTime: 2021-02-09 23:07:56
 * @LastEditors: Xu.WANG
 * @Description: 
...
#define KIRI_2PI 6.283185307179586476925286766559005768394f    // 2*PI
#define KIRI_SQRTPI 1.772453850905516027298167483341145182797f // sqrt(PI)

#define KIRI_SQRT2PI 2.506628274631000502415765284811045253006f // sqrt(2*PI)
...
{
    // Helper Function
    static inline __host__ __device__ uint CuCeilDiv(const uint a, const uint b) { return (a % b != 0) ? (a / b + 1) : (a / b); }
    ...
    template <class Type>
    using IsSame_Float2 = std::is_same<Type, float2>;
    template <class Type>
    using IsSame_Float3 = std::is_same<Type, float3>;
    template <class Type>
    using IsSame_Float4 = std::is_same<Type, float4>;
    template <class Type>
    using IsSame_Int = std::is_same<Type, int>;
    template <class Type>
    using IsSame_UInt = std::is_same<Type, uint>;
