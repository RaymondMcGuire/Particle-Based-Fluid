 ...
 * @LastEditTime: 2021-02-04 17:23:50
 * @LastEditors: Xu.WANG
 * @Description: 
...
inline __host__ __device__ float3 ones()
{
    return make_float3(1.f);
}

inline __host__ __device__ float3 zeros()
{
 ...
 */
__host__ __device__ inline tensor3x3 inertiaTensorTranslate(tensor3x3 I, float m, float3 off)
{
...
template <typename T>
__host__ __device__ inline int sgn(T val)
{
    return (T(0) < val) - (val < T(0));
}

inline __host__ __device__ float3 sgn(float3 v) { return make_float3(sgn(v.x), sgn(v.y), sgn(v.z)); }
...
inline __host__ __device__ bool edge_aabb(const float3 p0, const float3 p1, const float3 p2,
                                          const float3 f, const float3 l)
{
    float3 e0 = make_float3(1.f, 0.f, 0.f), e1 = make_float3(0.f, 1.f, 0.f), e2 = make_float3(0.f, 0.f, 1.f);
    ...
    if (fminf3(ap0) > r0 || fmaxf3(ap0) < -r0 || fminf3(ap1) > r1 || fmaxf3(ap1) < -r1 || fminf3(ap2) > r2 || fmaxf3(ap2) < -r2)
    {
        return true;
    }
 ...
 * @return {*}
 */
inline __host__ __device__ bool triangle_aabb(const float3 p0, const float3 p1, const float3 p2, const float3 n,
                                              const float3 c, const float3 l)
{
    float3 rp0, rp1, rp2;
    rp0 = p0 - c;
    ...
    // AABB Box
    float3 bmin = make_float3(-l.x, -l.y, -l.z), bmax = make_float3(l.x, l.y, l.z);
    ...
    if (edge_aabb(rp0, rp1, rp2, f0, l) || edge_aabb(rp0, rp1, rp2, f1, l) || edge_aabb(rp0, rp1, rp2, f2, l))
    {
        return false;
    }
...
inline __host__ __device__ bool polygon_intersects_cube(const float3 v0, const float3 v1, const float3 v2, const float3 n, const float3 c)
{
