 ...
 * @Date: 2022-08-11 11:44:25
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2022-09-03 21:08:52
 * @FilePath: \Kiri\KiriExamples\src\sph\multisph_ren14_app.cpp
 * @Description:
...
namespace KIRI
{
  void KiriMultiSphRen14App::SetupPBSParams()
  {
    ...
    CUDA_MULTISPH_REN14_PARAMS.phase_num = multisph_data->phase_num();

    for (size_t i = 0; i < CUDA_MULTISPH_REN14_PARAMS.phase_num; i++)
    {
      CUDA_MULTISPH_REN14_PARAMS.rho0[i] = multisph_data->rho0()->Get(i);
      CUDA_MULTISPH_REN14_PARAMS.mass0[i] = multisph_data->mass0()->Get(i);
      CUDA_MULTISPH_REN14_PARAMS.visc0[i] = multisph_data->visc0()->Get(i);
      CUDA_MULTISPH_REN14_PARAMS.color0[i] = FbsToKiriCUDA(
          *(multisph_data->color0()->GetAs<FlatBuffers::float3>(i)));

      KIRI_LOG_INFO(
          "MultiSph Phase {0}: rho={1},mass={2},visc={3},color=({4},{5},{6})", i,
          CUDA_MULTISPH_REN14_PARAMS.rho0[i], CUDA_MULTISPH_REN14_PARAMS.mass0[i],
          CUDA_MULTISPH_REN14_PARAMS.visc0[i],
          CUDA_MULTISPH_REN14_PARAMS.color0[i].x,
          CUDA_MULTISPH_REN14_PARAMS.color0[i].y,
          CUDA_MULTISPH_REN14_PARAMS.color0[i].z);
    }

    // sph
    CUDA_MULTISPH_REN14_PARAMS.kernel_radius = multisph_data->kernel_radius();
    CUDA_MULTISPH_REN14_PARAMS.particle_radius = multisph_data->particle_radius();

    KIRI_LOG_INFO(
        "particle_radius= {0}", CUDA_MULTISPH_REN14_PARAMS.particle_radius);

    CUDA_MULTISPH_REN14_PARAMS.stiff = multisph_data->stiff();
    CUDA_MULTISPH_REN14_PARAMS.sound_speed = multisph_data->sound_speed();
    CUDA_MULTISPH_REN14_PARAMS.bnu = multisph_data->bnu();

    CUDA_MULTISPH_REN14_PARAMS.gravity = FbsToKiriCUDA(*multisph_data->gravity());
    CUDA_MULTISPH_REN14_PARAMS.dt = multisph_data->fixed_dt();

    // ren2014
    CUDA_MULTISPH_REN14_PARAMS.tou = multisph_data->tou();
    CUDA_MULTISPH_REN14_PARAMS.sigma = multisph_data->sigma();
    CUDA_MULTISPH_REN14_PARAMS.miscible = multisph_data->miscible();
    ...
    if (app_data->render_mode_enable())
      ...
      SetRenderFps(1.f / CUDA_MULTISPH_REN14_PARAMS.dt);
    ...
    for (size_t i = 0; i < CUDA_MULTISPH_REN14_PARAMS.phase_num; i++)
    {
      ...
      volume_emitter->buildMultiSphRen14Volume(
          ...
          CUDA_MULTISPH_REN14_PARAMS.particle_radius,
          CUDA_MULTISPH_REN14_PARAMS.color0[i],
          CUDA_MULTISPH_REN14_PARAMS.mass0[i], i);
    }
    ...
    auto fluid_particles = std::make_shared<CudaMultiSphRen14Particles>(
        CUDA_MULTISPH_APP_PARAMS.max_num, volume_data.pos, volume_data.col,
        volume_data.mass, volume_data.phaseLabel, CUDA_MULTISPH_REN14_PARAMS.rho0,
        CUDA_MULTISPH_REN14_PARAMS.mass0, CUDA_MULTISPH_REN14_PARAMS.visc0,
        CUDA_MULTISPH_REN14_PARAMS.color0);

    auto sph_solver_type = scene_config_data->sph_solver_type();
    bool adaptive_sub_timestep = false;
    CudaMultiSphRen14SolverPtr solver;
    CUDA_MULTISPH_REN14_PARAMS.solver_type = SPH_SOLVER;
    switch (sph_solver_type)
    {
    case FlatBuffers::CudaSphType::CudaSphType_SPH:
      solver =
          std::make_shared<CudaMultiSphRen14Solver>(fluid_particles->maxSize());
      break;
    case FlatBuffers::CudaSphType::CudaSphType_WCSPH:
      adaptive_sub_timestep = true;
      CUDA_MULTISPH_REN14_PARAMS.solver_type = WCSPH_SOLVER;
      solver =
          std::make_shared<CudaMultiWCSphRen14Solver>(fluid_particles->maxSize());
      break;
    default:
      solver =
          std::make_shared<CudaMultiSphRen14Solver>(fluid_particles->maxSize());
    ...
    CudaGNSearcherPtr searcher = std::make_shared<CudaGNSearcher>(
        ...
        SearcherParticleType::MULTISPH_REN14);
    ...
    mSystem = std::make_shared<CudaMultiSphRen14System>(
  ...
  void KiriMultiSphRen14App::OnPBSUpdate(const KIRI::KiriTimeStep &DeltaTime)
  {
  ...
  void KiriMultiSphRen14App::SetupPBSScene()
  {
    ...
    // wall(no material)
    KiriBoxPtr boxModel = std::make_shared<KiriBox>(
    ...
    KiriEntityPtr entity_debug =
        std::make_shared<KiriEntity>(id++, boxModel, m_debug);

    mScene->Add(entity_roomtile);
    mScene->Add(entity_debug);
  }

  void KiriMultiSphRen14App::OnImguiRender()
  {
    ...
    if (p_open)
    {
      ...
      if (ImGui::Begin("SPH Examples", &p_open))
      {
        ...
        {
          const char *items[] = {"multisph_ren2014_non_miscible",
                                 "multisph_ren2014_miscible"};
