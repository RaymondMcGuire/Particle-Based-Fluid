 ...
 * @LastEditTime: 2022-12-21 17:06:15
 * @FilePath:
 * \Particle-Based-Fluid-Toolkit\simulator_cuda\include\kiri_pbs_cuda\solver\sph\cuda_dfsph_solver_gpu.cuh
 * @Description:
...
__global__ void
_ComputeAlpha_CUDA(float *alpha, const float3 *pos, const float *mass,
...
__global__ void _ComputeDivgenceError_CUDA(
    float *stiff, float *densityError, const float *alpha, const float3 *pos,
...
__global__ void _CorrectDivergenceByJacobi_CUDA(
    float3 *vel, const float *stiff, const float3 *pos, const float *mass,
...
__global__ void _ComputeDensityError_CUDA(
    float *densityError, float *stiff, const float *alpha, const float3 *pos,
    const float3 *vel, const float *mass, const float *density,
    const float rho0, const float dt, const size_t num, size_t *cellStart,
    float3 *bPos, float *bVolume, size_t *bCellStart, const int3 gridSize,
    Pos2GridXYZ p2xyz, GridXYZ2GridHash xyz2hash, GradientFunc nablaW) {
...
#pragma unroll
  for (size_t m = 0; m < 27; __syncthreads(), ++m) {
    ...
    _AdaptVelocitiesByPressure(&vel[i], i, stiff, pos, mass, dt,
                               cellStart[hash_idx], cellStart[hash_idx + 1],
                               nablaW);
