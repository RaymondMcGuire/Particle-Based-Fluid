...
namespace KIRI
{
                                        ...
                                        const size_t cellEnd, Func W)
  {
    ...
    while (j < cellEnd)
    {
      *a += -visc * 2.f * mass[j] / (density[i] + density[j]) * (vel[i] - vel[j]) *
            W(length(pos[i] - pos[j]));
      ++j;
    }
    return;
  }
  ...
  template <typename GradientFunc>
  __device__ void
  _ComputeBoundaryArtificialViscosity(float3 *a, const float3 posI, const float3 *posB,
                                      const float3 velI, const float densityI,
                                      const float *volumeB, const float bnu, const float kernelRadius,
                                      const float rho0, const size_t cellStart,
  ...
  template <typename GradientFunc>
  __device__ void _ComputeArtificialViscosity(
      float3 *a,
      const size_t i,
      const float3 *pos,
      const float3 *vel,
  ...
  __global__ void
  _ComputeDensity_CUDA(float *density, const float3 *pos, const float *mass,
                       const float rho0, const size_t num, const size_t *cellStart,
                       const float3 *bPos, const float *bVolume,
                       const size_t *bCellStart, const int3 gridSize,
...
#pragma unroll
    for (int m = 0; m < 27; __syncthreads(), ++m)
    {
      ...
      _ComputeArtificialViscosity(&a, i, pos, vel, mass, density, nu, kernelRadius,
                                  cellStart[hash_idx], cellStart[hash_idx + 1],
                                  nablaW);
      _ComputeBoundaryArtificialViscosity(&a, pos[i], bPos, vel[i], density[i], bVolume,
                                          bnu, kernelRadius, rho0, bCellStart[hash_idx],
