 ...
 * @LastEditTime: 2022-08-27 11:55:53
...
namespace KIRI
{
  ...
  void CudaVolumeEmitter::buildSphVolume(SphVolumeData &data, float3 lowest,
                                         ...
                                         float particleMass, float3 color)
  {
    if (!bEnable)
      return;
    ...
    for (size_t i = 0; i < static_cast<size_t>(vsize.x); ++i)
    {
      for (size_t j = 0; j < static_cast<size_t>(vsize.y); ++j)
      {
        for (size_t k = 0; k < static_cast<size_t>(vsize.z); ++k)
        {
          ...
          data.mass.emplace_back(particleMass);
  ...
  void CudaVolumeEmitter::buildSphShapeVolume(SphVolumeData &data,
                                              Vector<float4> shape,
                                              float particleMass,
                                              float3 color)
  {
                                              ...
                                              Vec_Float mass, float3 color)
  {
    ...
    for (size_t i = 0; i < shape.size(); i++)
    {
      ...
      data.col.emplace_back(color);
      data.radius.emplace_back(radius);
      data.mass.emplace_back(mass[i]);
      data.minRadius = std::min(radius, data.minRadius);
      data.maxRadius = std::max(radius, data.maxRadius);
    }
  }

  void CudaVolumeEmitter::buildMultiSphRen14Volume(MultiSphRen14VolumeData &data,
  ...
  void CudaVolumeEmitter::buildMultiSphYan16Volume(MultiSphYan16VolumeData &data,
                                                   float3 lowest, int3 vsize,
                                                   float particleRadius,
                                                   float3 color, float mass,
                                                   size_t phaseIdx,
  ...
  void CudaVolumeEmitter::buildMRDemVolume(DemShapeVolumeData &data,
                                           float3 lowest, int3 vsize,
                                           float particleRadius, float3 color,
                                           float mass, float jitter)
  {
    if (!bEnable)
      return;
  ...
  void CudaVolumeEmitter::buildSeepageflowShapeVolume(
      SeepageflowVolumeData &data, Vector<float4> shape, float3 color,
  ...
  void CudaVolumeEmitter::buildSeepageflowShapeMultiVolume(
      SeepageflowMultiVolumeData &data, Vector<float4> shape, float3 color,
