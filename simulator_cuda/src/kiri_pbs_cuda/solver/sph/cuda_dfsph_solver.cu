 ...
 * @Date: 2022-07-25 01:13:36
 * @LastEditors: Xu.WANG raymondmgwx@gmail.com
 * @LastEditTime: 2022-07-25 01:13:36
 * @FilePath: \Kiri\KiriPBSCuda\src\kiri_pbs_cuda\solver\sph\cuda_iisph_solver
 * copy.cu
 * @Description:
...
namespace KIRI
{
        ...
        const float kernelRadius, const int3 gridSize)
    {
        ...
        _ComputeDensity_CUDA<<<mCudaGridSize, KIRI_CUBLOCKSIZE>>>(
            data->densityPtr(), data->posPtr(), data->massPtr(), rho0,
        ...
        const float kernelRadius, const int3 gridSize)
    {
        ...
        while ((total_error > mDivergenceErrorThreshold * num * rho0 || (iter < mDivergenceMinIter)) &&
        ...
        const float kernelRadius, const int3 gridSize)
    {
        ...
        KIRI_CUCALL(cudaMemcpy(data->warmStiffPtr(), data->stiffPtr(), sizeof(float) * num, cudaMemcpyDeviceToDevice));

        auto iter = 0;
        auto total_error = std::numeric_limits<float>::max();

        while ((total_error > mPressureErrorThreshold * num * rho0 || (iter < mPressureMinIter)) && (iter < mPressureMaxIter))
        {
            ...
            thrust::transform(thrust::device,
                              data->warmStiffPtr(), data->warmStiffPtr() + num,
                              data->stiffPtr(),
                              data->warmStiffPtr(),
                              thrust::plus<float>());
            iter++;
            ...
            if (iter >= mPressureMinIter)
            {
                ...
                // printf("pressure iter=%d, total_error=%.6f \n", iter,
                //        total_error);
            }
        }
