...
KiriMesh *KiriModelLoadObj::ProcessMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    return new KiriMesh(vertices, indices, textures, instancing, trans4);
}
...
Array1<Texture> KiriModelLoadObj::LoadMaterialTextures(aiMaterial *mat, aiTextureType type, String typeName)
{
    ...
    for (UInt i = 0; i < mat->GetTextureCount(type); i++)
    {
        ...
        { // if texture hasn't been loaded already, load it
            ...
            textures_loaded.append(texture); // store it as texture loaded for entire model, to ensure we won't unnecesery load duplicate textures.
        }
    }
    return textures;
}
