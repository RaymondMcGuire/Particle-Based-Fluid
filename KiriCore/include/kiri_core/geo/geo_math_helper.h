 ...
 * @LastEditTime: 2020-10-21 23:05:39
 * @LastEditors: Xu.WANG
 * @Description: 
 * @FilePath: \Kiri\KiriCore\include\kiri_core\geo\geo_math_helper.h
 * @Reference:https://ttnghia.github.io ; Banana
 */

#ifndef _KIRI_GEO_MATH_HELPER_H_
#define _KIRI_GEO_MATH_HELPER_H_
#pragma once
#include <kiri_pch.h>
...
#define M_PI 3.1415926535897932384626433832795
    ...
    constexpr VectorX<N, T> min(const VectorX<N, T> &a, const VectorX<N, T> &b)
    {
        VectorX<N, T> result;
        for (Int d = 0; d < N; ++d)
        {
    ...
    template <Int N, class T>
    VectorX<N, T> max(const VectorX<N, T> &a, const VectorX<N, T> &b)
    {
        VectorX<N, T> result;
        for (Int d = 0; d < N; ++d)
        {
    ...
    template <Int N, class T>
    inline VectorX<N, T> clamp(const VectorX<N, T> &x, const VectorX<N, T> &a, const VectorX<N, T> &b)
    {
        VectorX<N, T> result;
        for (Int d = 0; d < N; ++d)
        {
    ...
    template <Int N, class T>
    inline VectorX<N, T> clamp(const VectorX<N, T> &x, const VectorX<N, T> &a, const VectorX<N, T> &b, T margin)
    {
        VectorX<N, T> result;
        for (Int d = 0; d < N; ++d)
        {
    ...
    template <Int N, class T>
    inline void clampInPlace(VectorX<N, T> &x, const VectorX<N, T> &a, const VectorX<N, T> &b)
    {
        for (Int d = 0; d < N; ++d)
        {
    ...
    template <Int N, class T>
    inline void clampInPlace(VectorX<N, T> &x, const VectorX<N, T> &a, const VectorX<N, T> &b, T margin)
    {
        for (Int d = 0; d < N; ++d)
        {
    ...
    inline UInt round_up_to_power_of_two(UInt n)
    {
        Int exponent = 0;
        --n;
    ...
    inline UInt round_down_to_power_of_two(UInt n)
    {
        Int exponent = 0;
    ...
    template <Int N, class T>
    inline void get_barycentric(const VectorX<N, T> &x, VectorX<N, Int> &i, VectorX<N, T> &f)
    {
        for (Int j = 0; j < N; ++j)
        {
            T s = std::floor(x[j]);
