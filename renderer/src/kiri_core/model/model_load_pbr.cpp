 ...
 * @LastEditTime: 2021-03-25 14:18:03
 * @LastEditors: Xu.WANG
 * @Description:
...
Array1<Vector2F> KiriModelLoadPBR::Project2Screen(KIRI::KiriCameraPtr camera)
{
    Array1<Vector2F> out;
    float outWidth = 1920.f;
    float outHeight = 1080.f;
    for (size_t i = 0; i < mVertexs.size(); i++)
    {
        auto vert = mVertexs[i];
        auto p3 = Vector3F(vert.Position[0], vert.Position[1], vert.Position[2]) - mBBoxMin;

        // auto proj = camera->ProjectionMatrix() * p3;
        // proj /= proj.w;
        out.append(Vector2F(p3.x, p3.y));
        // out.append(Vector2F(proj.x * outWidth / 2.f + outWidth / 2.f, proj.y * outHeight / 2.f + outHeight / 2.f));
    }
...
bool KiriModelLoadPBR::AkinciTriangleSampling(const Vector3F &Point1, const Vector3F &Point2, const Vector3F &Point3, const float &Radius, Array1Vec3F &Samples)
{
    ...
    Vector3F cross, mNormal;
    cross = edgesV[lEdge].cross(edgesV[sEdge]);
    mNormal = edgesV[sEdge].cross(cross);
    mNormal.normalize();
    ...
    float sign = mNormal.dot(tmpVec);
    if (sign < 0)
        mNormal = -mNormal;

    float triangleHeight = std::abs(mNormal.dot(edgesV[lEdge]));
    ...
    {
        sweepA = v[(Int)edgesI[sEdge][0]] + (float)i * (2.f * Radius) * mNormal;
        sweepB = v[(Int)edgesI[sEdge][1]] + (float)i * (2.f * Radius) * mNormal;
...
KiriMeshPBRPtr KiriModelLoadPBR::ProcessMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    ReMakeVertexPosition(mOffset, mScale, false);
