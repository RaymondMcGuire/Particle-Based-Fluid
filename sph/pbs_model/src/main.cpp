...
int main(){
    ...
    pbs_obj.Save("out/cube.obj",cube.getVertices(),cube.getNormals(),pbs_face_vn);
    cout<<"write cube.obj to file"<<endl;

	Sphere sphere = Sphere(0.1f, 36, 18, true);
    pbs_face_vtn.n_indices = sphere.getNormIndices();
    pbs_face_vtn.v_indices = sphere.getVertIndices();

    pbs_obj.Save("out/sphere.obj",sphere.getVertices(),sphere.getNormals(),pbs_face_vtn);
